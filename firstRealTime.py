...
def findStopById(target_trip_id: str, target_stop_id: str):
    for entity in feed.entity:
        if entity.HasField("trip_update"):
            if (
                entity.trip_update.trip.trip_id == target_trip_id
            ):  # recherche de la section comportant l'id du trip souhaité
                for stop_time_update in entity.trip_update.stop_time_update:
                    if (
                        stop_time_update.stop_id == target_stop_id
                    ):  # recherche de la sous-section comportant l'id de l'arrêt souhaité
        ...
        if entity.HasField("trip_update"):
            for stop_time_update in entity.trip_update.stop_time_update:
                if (
                    ...
                    if (
                        str(stop_time_update.schedule_relationship) == "0"
                    ):  # 0=SCHEDULED, 1=SKIPPED
                        average_delay = average_delay + int(
