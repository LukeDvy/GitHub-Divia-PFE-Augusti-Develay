...
def recupRouteId(tripId: str):
    ...
    result = result[
        result["trip_id"].astype(str) == str(tripId)
    ]
    print("Trajet "
            + str(result["route_id"].loc[1])
        )
...
def miseEnForme(routeId: str,tripId: str, directionId: int):
    ...
    result = result[
        result["direction_id"] == directionId
    ]
    ...
    for index, row in result.iterrows():
            result.loc[index, "departure_time"] = datetime.datetime.fromtimestamp(row["arrival_time"])
    print("Trajet "
